...
function readDomState(): Partial<PreferencesState> {
  ...
  return {
    ...
    currentRole: root.getAttribute("data-role") || "public",
  };
}
...
export const PreferencesStoreProvider = ({
  ...
  currentRole: PreferencesState["currentRole"];
}) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeStore = store.subscribe((s, p) => {
      ...
      if (s.currentRole !== p.currentRole) {
        if (typeof window !== "undefined") {
          localStorage.setItem("bytestart_role", s.currentRole);
          document.cookie = `bytestart_role=${s.currentRole}; path=/; max-age=31536000; SameSite=Lax`;
          document.documentElement.setAttribute("data-role", s.currentRole);
        }
      }
    });
