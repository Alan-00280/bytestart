    ...
    if (savedCourses) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          localStorage.setItem("bytestart_owner_courses", JSON.stringify(parsedCourses));
        }
      } catch (e) {}
    }
  ...
  if (!mounted || !selectedCourse) {
    return (
      <div className="bg-slate-950 min-h-screen text-white flex flex-col items-center justify-center p-6 font-sans">
        ...
        <p className="text-white/60 mb-6 text-sm">Maaf, kelas dengan ID tersebut tidak tersedia di database studio Anda.</p>
        <button 
          onClick={handleBack}
          className="bg-amber-500 hover:bg-amber-600 text-slate-950 text-sm font-semibold px-6 py-2.5 rounded-full transition-all"
        >
          Kembali ke Studio
        </button>
      </div>
      ...
      <div className="flex flex-col gap-6 bg-slate-950/20 rounded-2xl border border-white/[0.06] p-6 backdrop-blur-xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.05] pb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[9px] font-bold text-amber-500 uppercase tracking-widest leading-none font-mono">Course Studio Workspace</span>
